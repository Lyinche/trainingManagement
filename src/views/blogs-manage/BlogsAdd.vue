<template>
    <div>
        <el-page-header icon="" title="内容管理" content="创建内容" />
        <el-form ref="blogsFormRef" :model="blogsForm" :rules="blogsFormRules" label-width="80px" class="demo-ruleForm">
            ...
            <!-- 创建内容 -->
            <el-form-item>
                <el-button class="submit" type="primary" @click="submit()">发布内容</el-button>
            </el-form-item>
        </el-form>
    </div>
</template>
...
const submit = () => {
    blogsFormRef.value.validate(async (valid) => {
        ...
        if (valid) {
            let res = await upData("/adminApi/blog/addBlog", blogsForm);
            ElNotification({
                title: '创建成功',
                message: '创建新内容成功!马上跳转到内容列表页面!',
                type: 'success',
            })
            setTimeout(() => {
                router.push('/blogsList');
            }, 2000);
        } else {
            ElNotification({
                title: '创建失败',
